...
(`.pages`, `.numbers`, `.key`) even when their identifying data lives deeper
inside the archive than the cached bytes. iWork classification reads a bounded
portion of `Index/Document.iwa`; shared ZIP entry names alone cannot
distinguish Pages from Numbers.
